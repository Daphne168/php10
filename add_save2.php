<?php
include 'config.php';      //db設定
include 'utility.php';     //常用的函式寫在這一支
...
// bindParam 綁定參數，綁定在一個變數，這個變裡的值可以變。
$sth = $pdo->prepare($sqlstr);
$sth->bindParam(':usercode', $usercode, PDO::PARAM_STR);
$sth->bindParam(':username', $username, PDO::PARAM_STR);
$sth->bindParam(':address' , $address , PDO::PARAM_STR);
$sth->bindParam(':birthday', $birthday, PDO::PARAM_STR);
$sth->bindParam(':height'  , $height  , PDO::PARAM_INT);   //數字是INT
$sth->bindParam(':weight'  , $weight  , PDO::PARAM_INT);   //數字是INT
...
// bindValue 綁定值，一旦綁定了，那個值就變常數了，不會再改變。
...
if($sth->execute())
{
   ...
   $url_display = 'display2.php?uid=' . $new_uid;
